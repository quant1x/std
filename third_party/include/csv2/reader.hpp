  ...
  public:
    ...
    template <typename Container> void read_value(Container &result) const {
      if (start_ >= end_)
        return;
      //result.reserve(end_ - start_);
      size_t _start = start_;
      size_t _end = end_;
      //std::cout << ">>1:s[" << buffer_[_start] <<"]"<< std::endl;
      //std::cout << ">>1:e[" << buffer_[_end-1] <<"]"<< std::endl;
      if(buffer_[_start] == quote_character::value && buffer_[_end-1] == quote_character::value) {
        //std::cout << ">>1-1"<< std::endl;
        ++_start;
        --_end;
      }
      result.reserve(_end - _start);
      //std::cout << ">>2"<< std::endl;
      const auto new_start_end = trim_policy::trim(buffer_, _start, _end);
